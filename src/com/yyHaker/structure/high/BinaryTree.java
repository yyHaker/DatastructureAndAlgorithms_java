 ...
 * 二叉搜索树
 *
 *博客地址： http://blog.csdn.net/eson_15/article/details/51138663
 *
 * @author Le Yuan
 * @date 2016/11/24
 */
public class BinaryTree {
    ...
    public boolean delete(int key) {
        BNode current = root;
//      BNode parent = root;
        boolean isLeftChild = true;  //记录当前节点是其父节点的左孩子还是右孩子
...
//          parent = current;
    ...
    public boolean deleteOneChild(BNode node, boolean isLeftChild) {
        if(node.leftChild == null) {//要删除的节点有一个右孩子
            if(node == root) {
            ...
            node.rightChild.parent = node.parent;  //设置父节点
        } else {                        //要删除的节点有一个左孩子
            if(node == root) {
                root = node.leftChild;
                node.parent = null;
            ...
            node.leftChild.parent = node.parent;  //设置父节点
        }
        return true;
    }
...
class BNode {
    public int key;          //key是用来比较的，value是真正的值，可以只用一个
    public double data;
    public BNode parent;
